...
class Pointers {
 public:
  Pointers(LAMMPS *ptr) : //constructor. Instance of other classes (e.g. momery, atom...) are member variables of class Pointer, so should be listed here.
    lmp(ptr),
    memory(ptr->memory),
    error(ptr->error),
 ...
 protected:
  ...
  Atom *&atom; // pointer parameter by reference
  Update *&update;
  Neighbor *&neighbor;
  Comm *&comm;
  Domain *&domain;
