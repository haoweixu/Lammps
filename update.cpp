...
/*------------------modified by Haowei Xu (MIT)--------------------
	energy:		        MeV
  distance:         femto-meter
  time:             atto-second (10^(-18) s)
  temperture:       kelvin
  velocity:         fm / as
  force:            MeV / fm
  density:          g/cm^dim
  dyna-viscosity:   Poise
  electric field :  Mega-Volt / fm
  pressure:         barn
------------------------------------------------------------------*/  
  else if (strcmp(style, "nuclear") == 0) {
    force->boltz = 86.17342791e-12;
    force->hplanck = 4.135667403e-3;
    force->mvv2e = 1.0364269e-8;
    force->ftm2v = 1.0/1.0364269e-8;
    force->mv2d = 1.672621e15;
    force->nktv2p = 1.6021765e27;
    force->qqr2e = 1.4399645;
    force->qe2f = 1.0;
    force->vxmu2f = 6.241509647e-16;
    force->xxt2kmu = 1.0e-8;
    force->e_mass = 0.0;    // not yet set
    force->hhmrr2e = 0.0;
    force->mvh2r = 0.0;
    force->angstrom = 1.0e5;
    force->femtosecond = 1.0e3;
    force->qelectron = 1.0;
