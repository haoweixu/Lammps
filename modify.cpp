...
#include "style_fix.h" // seems that this file is created after compilation
#undef FixStyle
#undef FIX_CLASS
  ...
  if (trysuffix && lmp->suffix_enable) {
    if (lmp->suffix) {
      ...
      if (fix_map->find(estyle) != fix_map->end()) {
        ...
        fix[ifix] = fix_creator(lmp,narg,arg); // functor (function object) ?
    ...
    if (fix[ifix] == NULL && lmp->suffix2) {
      ...
      sprintf(estyle,"%s/%s",arg[2],lmp->suffix2);
      if (fix_map->find(estyle) != fix_map->end()) {
        FixCreator fix_creator = (*fix_map)[estyle]; // fix_creator is a pointer to functions
