...
DA_tracer::DA_tracer(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
  if (narg < 9) error->all(FLERR,"Illegal fix DA_tracer command. Fix 3 all DA_tracer id_pe NumALL xratio yratio zratio tracer_type seed");
  global_freq = 1;
...
void DA_tracer::init()
{
  ...
  if (icompute < 0) error->all(FLERR,"PE ID for fix DA_tracer does not exist");
  pe = modify->compute[icompute]; 
}

/* ---------------------------------------------------------------------- */

void DA_tracer::setup(int vflag)
{
  post_force(vflag);
}

void DA_tracer::min_setup(int vflag)
{
  post_force(vflag);
}
...
void DA_tracer::post_force(int vflag)
{
  int i;
  double EngCur = 0;
  double factor = 1.0;
  ...
  if (array_buf == NULL) memory->create(array_buf,natoms,"DA_tracer:array_buf");

  int *recvcounts,*displs;

  // gather the tag & property DA_tracerta info of all the atoms on each proc
  ...
  if (natoms != atom->natoms && comm->me == 0) {
    char str[128];
    sprintf(str,"Lost atoms via DA_tracer: original " BIGINT_FORMAT
...
void DA_tracer::min_post_force(int vflag)
{
  post_force(vflag);
}
...
double DA_tracer::memory_usage()
{
  double bytes = 0.0;
  return bytes;
}

void DA_tracer::sort(double * a_one, bigint low, bigint high)
{
  bigint i, j;
  double t;
  if(low<high)
