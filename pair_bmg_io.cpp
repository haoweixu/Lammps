...
/* ----------------------------------------------------------------------
   ...
   Born-Mayer-Huggins potential with an inner and an outer cut
   
   V = V_BMG - V_BMG(r_in)      for r_in < r < r_out
     = 0                        for r < r_in or r > r_out

   at r = r_in, orginal BMG potential is turned off and shifted to 0, so that it can be connected to some other potential like Ziegler-Biersack-Littmark potential, which is turned on for r < r_in, and also shifted to 0 at r = r_in. In this way a relatively smooth transition is obtained.  


syntax:
		pair_style		bmg_io cut_outter
		pair_coeff		i j A rho sigma C D cut_inner (cut_outter)
...
PairBMGIO::PairBMGIO(LAMMPS *lmp) : Pair(lmp)
{
  writedata = 1;
}
...
void PairBMGIO::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype] && rsq > cut_inner_sq[itype][jtype]) {
        r2inv = 1.0/rsq;
...
void PairBMGIO::settings(int narg, char **arg)
{
...
void PairBMGIO::coeff(int narg, char **arg)
{
  ...
  double cut_inner_one = force->numeric(FLERR, arg[7]);
  ...
  for (int i = ilo; i <= ihi; i++) {
    for (int j = MAX(jlo,i); j <= jhi; j++) {
      ...
      cut_inner_sq[i][j] = cut_inner_one*cut_inner_one;
      cut[i][j] = cut_one;
      setflag[i][j] = 1;
      count++;
    }
...
double PairBMGIO::init_one(int i, int j)
{
  ...
  if (cut[i][j] > 0.0) {
    double rexp = exp((sigma[i][j]-cut_inner[i][j])*rhoinv[i][j]);
    offset[i][j] = a[i][j]*rexp - c[i][j]/pow(cut_inner[i][j],6.0) +
      d[i][j]/pow(cut_inner[i][j],8.0);
  } else offset[i][j] = 0.0;
...
void PairBMGIO::write_restart(FILE *fp)
{
...
void PairBMGIO::read_restart(FILE *fp)
{
...
void PairBMGIO::write_restart_settings(FILE *fp)
{
...
void PairBMGIO::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairBMGIO::write_data(FILE *fp)
{
...
void PairBMGIO::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
double PairBMGIO::single(int i, int j, int itype, int jtype,
...
void *PairBMGIO::extract(const char *str, int &dim)
{
  dim = 2;
