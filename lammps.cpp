...
/*-----------------------Haowei--------------------------------
members of class LAMMPS are pointers to other classes, e.g. atom.
So they do not need to be initialized when initializing LAMMPS.
They can be created later in create()
--------------------------------------------------------------*/
{
  ...
  // open input script
    ...
    if (me == 0) {
      infile = fopen(arg[inflag],"r"); // infile in MPI
      if (infile == NULL) {
        char str[128];
        sprintf(str,"Cannot open input script %s",arg[inflag]);
        error->one(FLERR,str);
