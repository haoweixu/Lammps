...
FixStyle(adjust_ts,FixAdjustTimeStep)

#else // register the fix here

#ifndef LMP_FIX_ADJUSTTIMESTEP_H
#define LMP_FIX_ADJUSTTIMESTEP_H

#include "fix.h"

namespace LAMMPS_NS {

class FixAdjustTimeStep : public Fix {
 public:
  FixAdjustTimeStep(class LAMMPS *, int, char **);
  ~FixAdjustTimeStep();
  int setmask();
  void init();
  void setup(int);
