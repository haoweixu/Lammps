...
class Modify : protected Pointers {
 ...
 public:
  ...
  typedef Fix *(*FixCreator)(LAMMPS *, int, char **); // FixCreator is pointer to functions
  typedef std::map<std::string,FixCreator> FixCreatorMap; // a map with string as key and FixCreator pointer as value.
 ...
 protected:
  ...
  // since fix_creator is a pointer to functions, it may point to different functions that require different returns. Here fix_creator is defined as a template function. Its return is of type T, which is in fact specified by which function fix_creator is pointing at. But all of them are inherited from class Fix, so the returned result can be assinged to a Fix pointer.
};
